...
class RoleService():
    ...
    def __init__(self, hand: set[Card], cards_on_table: set[Card]) -> None:
        cards: set[Card] = hand.union(cards_on_table)
        ...
        for card in cards:
            card_number_mapping[str(card.num)].append(card)
    ...
    def check_role(self) -> tuple[RoleType, list[Card]]:
        for cards in self._card_suit_mapping.values():
            if len(cards) == 5:
                sorted_cards = sorted(cards, key=lambda card: card.num)
                sorted_number = [card.num for card in sorted_cards]
...
def _get_highest_number_players(players: list[Player], highest_card_numbers: list[int] | None = None) -> list[Player]:
        if highest_card_numbers is None:
            highest_card_numbers = [player.role.cards[-1].num if player.role.cards[0] != 1 else 14 for player in players]
...
def _compare_highest_role_players(highest_role_players: list[Player]) -> list[Player]:
    ...
    if role_type == RoleType.ROYAL_FLUSH:
        ...
        highest_card_numbers = [player.role.cards[-1].num for player in highest_role_players]
